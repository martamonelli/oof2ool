...
#FIXME: to be passed from the slurm script
ptg_only = False
tod_only = True
...
in_map = hp.fitsfunc.read_map('/in/in_map_'+str(realization).zfill(2)+'.fits', field=[0,1,2])
...
if ptg_only:
    ...
    for (subfolder, components_to_bin) in [(tod_path, [])]:
        lbs.save_simulation_for_madam(
            ...
            components=["cmb-only_tod"], #list all the components
